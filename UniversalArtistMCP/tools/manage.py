...
def init():S.mkdir(exist_ok=True)
def config():return {'mcp':{'universal-artist':{'type':'local','command':[str(P),'-m','universal_artist.server'],'environment':{'UA_STATE_ROOT':str(S)}}}}
def doctor():
 r=subprocess.run([P,'-c','from universal_artist.server import create_server; create_server(); print("OK")'],capture_output=True,text=True);print(r.stdout or r.stderr);return r.returncode
def main():
